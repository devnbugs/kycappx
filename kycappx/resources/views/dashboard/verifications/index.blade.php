<x-layouts.dashboard-user title="Verifications" header="Verification Requests">
    <section class="surface-card p-6 sm:p-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            ...
            @if ($siteSettings->verification_enabled)
                <a href="{{ route('verifications.create') }}">
                    <x-ui.button>New Verification</x-ui.button>
                </a>
            @else
                <x-ui.button variant="secondary" disabled>Verification Disabled</x-ui.button>
            @endif
        </div>
    </section>
                    ...
                    @forelse ($verifications as $verification)
                        <tr class="table-row">
                            <td class="px-6 py-4">
                                ...
                                <div class="text-xs text-slate-500 dark:text-slate-400">{{ $verification->service?->code ?? 'N/A' }}</div>
                            </td>
