    ...
    <section class="surface-card p-6 sm:p-8">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div>
                ...
                <h2 class="mt-3 text-2xl font-semibold text-slate-950 dark:text-slate-50">Filter and inspect wallet activity</h2>
                ...
                @foreach (['all' => 'All', 'credit' => 'Credits', 'debit' => 'Debits', 'refund' => 'Refunds'] as $filter => $label)
                    <a
                        ...
                        @class([
                            ...
                            'bg-slate-100 text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700' => $type !== $filter,
                        ])
                    >
                        {{ $label }}
                    </a>
                ...
                <tbody>
                    @forelse ($transactions as $transaction)
                        <tr class="table-row">
                            ...
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ $transaction->description ?: 'No description provided' }}</td>
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ $transaction->created_at?->format('M d, Y H:i') }}</td>
                        </tr>
                    @empty
                        <tr class="table-row">
                            <td colspan="6" class="px-6 py-10 text-center text-slate-500 dark:text-slate-400">No transactions match this filter yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
