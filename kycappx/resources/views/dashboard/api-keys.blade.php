<x-layouts.dashboard-user title="API Keys" header="API Credential Management">
    <section class="grid gap-4 xl:grid-cols-[0.8fr,1.2fr]">
        <div class="surface-card p-6 sm:p-8">
            ...
            <h2 class="mt-3 text-2xl font-semibold text-slate-950">Issue a new API key</h2>
            ...
            @if (session('generated_api_key'))
                <div class="mt-6 rounded-[1.5rem] border border-amber-200 bg-amber-50 px-4 py-4">
                    <div class="text-sm font-semibold text-amber-900">Copy this key now. It will not be shown again.</div>
                    <div class="mt-3 break-all rounded-2xl bg-white px-4 py-3 font-mono text-sm text-slate-800">
        ...
        <div class="table-shell">
            <div class="px-6 py-5">
                ...
                <h2 class="mt-3 text-2xl font-semibold text-slate-950">API keys linked to this account</h2>
            </div>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <tbody>
                        @forelse ($apiKeys as $apiKey)
                            <tr class="table-row">
                                <td class="px-6 py-4 font-semibold text-slate-950">{{ $apiKey->name }}</td>
                                <td class="px-6 py-4 font-mono text-xs text-slate-700">{{ $apiKey->prefix }}…</td>
                                <td class="px-6 py-4 text-slate-600">{{ implode(', ', $apiKey->abilities ?? []) ?: 'No explicit abilities' }}</td>
                                <td class="px-6 py-4 text-slate-600">{{ $apiKey->last_used_at?->diffForHumans() ?? 'Never' }}</td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-right">
                                    ...
                                    @else
                                        <span class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">Inactive</span>
                                    @endif
                                </td>
                            </tr>
                        @empty
                            <tr class="table-row">
                                <td colspan="6" class="px-6 py-10 text-center text-slate-500">No API keys have been created for this account yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
