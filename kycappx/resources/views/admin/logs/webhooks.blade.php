<x-layouts.dashboard-admin title="Webhook Logs" header="Webhook Delivery Logs">
    <section class="surface-card p-6 sm:p-8">
        ...
        <h2 class="mt-3 text-2xl font-semibold text-slate-950 dark:text-slate-50">Signed callbacks, references, and processing status</h2>
                ...
                <tbody>
                    @forelse ($logs as $log)
                        <tr class="table-row">
                            ...
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ $log->event ?: 'Unknown event' }}</td>
                            ...
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ $log->error_message ?: 'No error' }}</td>
                            <td class="px-6 py-4 text-slate-600 dark:text-slate-300">{{ $log->created_at?->format('M d, Y H:i') }}</td>
                        </tr>
                    @empty
                        <tr class="table-row">
                            <td colspan="7" class="px-6 py-10 text-center text-slate-500 dark:text-slate-400">No webhook events have been captured yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
